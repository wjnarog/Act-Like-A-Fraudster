...
def scrape_property_info(address_to_search):
    ...
    search_term = "8724 Elgin Dr Lafayette"
    ...
    account_num_elem = soup.find(
        'span', class_='labelme', string="Account Number:")
    if account_num_elem:
        account_number = account_num_elem.find_next_sibling(
            'span').text.strip()
    ...
    mailing_addr_elem = soup.find(
        'span', class_='labelme', string="Mailing Address:")
    ...
    property_addr_elem = soup.find(
        'span', class_='labelme', string="Property Address:")
    if property_addr_elem:
        property_addr = property_addr_elem.find_next_sibling(
            'span').text.strip()
    ...
    if city_elem:
        ...
        state = city_elem.find_next_sibling('span').find_next_sibling(
            'span').find_next_sibling('span').text.strip()
    ...
    parcel_num_elem = soup.find(
        'span', class_='labelme', string="Parcel Number:")
    if parcel_num_elem:
    ...
    subdivison_elem = soup.find(
        'span', class_='labelme', string="Subdivision:")
    if subdivison_elem:
    ...
    market_area_elem = soup.find(
        'span', class_='labelme', string="Market Area:")
    if market_area_elem:
    ...
    for row_i in rows1:
        ...
        actual_value = row_i.find_all('td')[1].find(
            'span', class_='ng-binding').text.strip()
    ...
    full_bath_num_elem = soup.find(
        'span', class_='labelme', string="Full Bath:")
    if full_bath_num_elem:
        full_bath_num = full_bath_num_elem.find_next_sibling(
            'span').text.strip()
        property_details["Full Bath"] = full_bath_num
    else:
        property_details["Full Bath"] = "Not Found"

    three_qtr_bath_num_elem = soup.find(
        'span', class_='labelme', string="3/4 Bath:")
    if three_qtr_bath_num_elem:
        three_qtr_bath_num = three_qtr_bath_num_elem.find_next_sibling(
            'span').text.strip()
        property_details["3/4 Bath"] = three_qtr_bath_num
    else:
        property_details["3/4 Bath"] = "Not Found"

    half_bath_num_elem = soup.find(
        'span', class_='labelme', string="Half Bath:")
    if half_bath_num_elem:
        half_bath_num = half_bath_num_elem.find_next_sibling(
            'span').text.strip()
